...
category as (
  select 
    su.*, 
    ca.origin_flag ip_type
  from summary as su
  left join `hogeticlab-legs-prd.ip_dashbaord.dmart_ip_category` ca
  on su.large_class_ip_name = ca.large_class_ip_name
)
select distinct *
from category
where ip_type is not null and (ip_type ='漫画' or ip_type ='ライトノベル' or ip_type = 'ゲーム' or ip_type ='アニメ')
